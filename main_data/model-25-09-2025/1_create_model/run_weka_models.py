...
# --- ✨ แก้ไขส่วนนี้ตามการตั้งค่าล่าสุดที่ใช้งานได้ ---
WEKA_CLASSPATH = (
    "/opt/weka/weka-3-8-5/weka.jar:"
    "/opt/weka/weka-3-8-5/mtj-1.0.4.jar:"
    "/opt/weka/weka-3-8-5/netlib-all-1.1.2.jar"
)
...
def discover_dataset_pairs():
    ...
    if not found_pairs:
        print("  ❌ No valid train/test pairs found.")
    return found_pairs
...
# --- ✨ [แก้ไข] แก้ไขฟังก์ชันให้รับ parameter เพิ่มสำหรับโฟลเดอร์ model ---
def run_weka_with_tuning(train_file, test_file, model_save_subdir, data_model_subdir):
    ...
    for algo_class, _, file_base in ALGORITHMS:
        ...
        # โฟลเดอร์สำหรับเก็บผลลัพธ์ text (.txt) ยังคงอยู่ใน data_model
        ...
        for i, params in enumerate(param_combinations):
            ...
            run_name = f"{file_base}_{param_str}"
            
            # --- ✨ [แก้ไข] กำหนดให้ model_output บันทึกลงใน model_save_subdir ---
            model_output = os.path.join(model_save_subdir, f"{run_name}.model")
            result_output = os.path.join(algo_tuning_dir, f"{run_name}.txt")
...
def main():
    ...
    for train_file, test_file, output_prefix in file_configs:
        ...
        current_model_dir = os.path.join("model", output_prefix)
        ...
        if HYPERPARAMETER_TUNING_MODE:
            # --- ✨ [แก้ไข] ส่ง current_model_dir เพิ่มเข้าไปในฟังก์ชัน ---
            best_results = run_weka_with_tuning(train_file, test_file, current_model_dir, current_data_model_dir)
